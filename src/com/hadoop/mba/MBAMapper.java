...
public class MBAMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
   ...
   private static List<String> convertItemsToList(String line) {
      if ((line == null) || (line.length() == 0) || line.startsWith("order_id")) {
         // no mapper output will be generated
         return null;
      }      
      ...
      String[] item_tokens = StringUtils.split(tokens[1], ",");   
      if ( (item_tokens == null) || (item_tokens.length == 0) ) {
         return null;
      }
      List<String> items = new ArrayList<String>();         
      for (String item : item_tokens) {
         if (item != null) {
             items.add(item.trim());
         }         
      }         
